  ...
  return (
    <div className="app-container bg-gray-50">
      ...
      <div className="bg-white px-4 pt-10 pb-4 border-b border-gray-100">
        <h1 className="text-gray-900 font-bold text-lg">骑手配送中</h1>
            ...
            <div key={s.label} className="flex items-center">
              <div className="flex flex-col items-center">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
                  s.done ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-400'
                }`}>
                  {s.done ? '✓' : i + 1}
                </div>
                <span className={`text-xs mt-1 font-medium ${s.done ? 'text-orange-500' : 'text-gray-400'}`}>
                  {s.label}
                </span>
              </div>
              {i < progressSteps.length - 1 && (
                <div className={`w-10 h-0.5 mx-1 mb-4 transition-colors ${
                  progressSteps[i + 1].done ? 'bg-orange-500' : 'bg-gray-200'
                }`} />
              )}
            </div>
          ))}
      ...
      <div className="bg-white mx-4 mt-3 rounded-2xl p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-gray-900">赵雷</p>
              <div className="flex items-center gap-1">
                <span className="text-yellow-400 text-xs">★★★★★</span>
                <span className="text-gray-400 text-xs">4.9分 · 送单12万+</span>
              </div>
            </div>
          </div>
          <div className="flex gap-2">
            <button className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-xl">
              📞
            </button>
            <button className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-xl">
              💬
      ...
      {/* Restaurant info */}
      {restaurant && (
        <div className="bg-white mx-4 mt-3 rounded-2xl p-4 flex items-center gap-3">
          <div className="text-3xl">{restaurant.emoji}</div>
          <div className="flex-1">
            <p className="font-medium text-gray-900 text-sm">{restaurant.name}</p>
            <p className="text-gray-400 text-xs">骑手已取餐，正在前往您的位置</p>
          </div>
        </div>
      )}
      ...
      <div className="px-4 mt-4 pb-8">
        <button
          className="w-full border-2 border-gray-200 text-gray-600 py-4 rounded-2xl font-bold text-base active:scale-95 transition-transform"
